\chapter{Article Collection Pipeline}
\label{AppendixD}

This appendix documents the search, classification, and filtering process that produced the final sample of 53 articles. The pipeline combined bibliographic retrieval from OpenAlex, LLM-based classification of abstracts, automated replication package search across five repositories, and manual eligibility review. Figure~\ref{fig:collection_pipeline} summarizes the overall flow.

\section{Bibliographic Retrieval}
...
\section{Replication Package Search}
...
\section{Manual Review and Final Sample}
...
The review yielded 53 articles that met all criteria. Three additional candidates passed the manual review but were dropped when the reanalysis was attempted because their main specification could not be reproduced under the standardized protocol: one requires restricted-access mortality microdata that cannot be redistributed (\textcite{moorthy2025bluecollar}), one uses a multivariate policy-regressor baseline with state-specific linear trends (\textcite{myers2017power}), and one uses county-specific linear trends combined with interacted multi-dimensional fixed effects (\textcite{kuziemko2018managed}). These three articles are not part of the final sample. Of the 53 retained articles, 10 had been previously reanalyzed by hand and served as the calibration set described in Appendix~\ref{AppendixC}, §\ref{sec:appc_calibration}. The remaining 43 were processed through the semi-automated estimation pipeline described in Appendix~\ref{AppendixC}.
...
\subsection*{Prompt 1: DiD classification (initial search)}
...
\subsection*{Prompt 2: Dual health and DiD classification (full general search)}
...
\subsection*{Prompt 3: Aggressive DiD reclassification}
...
\subsection*{Prompt 4: Health topic verification (temperature = 0)}
