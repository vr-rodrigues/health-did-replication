...
The review yielded 53 articles that met all criteria. Three additional candidates passed the manual review but were dropped when the reanalysis was attempted because their main specification could not be reproduced under the standardized protocol: one requires restricted-access mortality microdata that cannot be redistributed (\textcite{moorthy2025bluecollar}), one uses a multivariate policy-regressor baseline with state-specific linear trends (\textcite{myers2017power}), and one uses county-specific linear trends combined with interacted multi-dimensional fixed effects (\textcite{kuziemko2018managed}). These three articles are not part of the final sample. Of the 53 retained articles, 11 had been previously reanalyzed by hand and served as the calibration set described in Section~\ref{sec:calibration}. The remaining 42 were processed through the semi-automated estimation pipeline described in Appendix~\ref{AppendixC}.
